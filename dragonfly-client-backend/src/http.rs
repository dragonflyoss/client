...
pub struct HTTP {
    ...
    /// request_header is the custom request headers configured in the dfdaemon config,
    /// which will be inserted into each request if the original header is not already set.
...
impl HTTP {
    ...
    // Apply custom request headers to the request header map.
    fn apply_request_headers(&self, request_header: &mut HeaderMap) -> Result<()> {
        // Apply the user agent if not specified in header.
        request_header
            .entry(USER_AGENT)
            .or_insert(HeaderValue::from_static(DEFAULT_USER_AGENT));

        // Apply custom request headers if provided and not defined in original request header.
...
impl super::Backend for HTTP {
    ...
    async fn head(&self, request: super::HeadRequest) -> Result<super::HeadResponse> {
        ...
        // Apply the custom request headers.
        self.apply_request_headers(&mut request_header)?;
...
"""#;
    ...
    #[test]
    fn should_apply_request_headers() {
        // Test 1: Apply default user-agent when not specified.
        let http = HTTP::new(HTTP_SCHEME, None).unwrap();
        let mut headers = HeaderMap::new();
        http.apply_request_headers(&mut headers).unwrap();
        assert_eq!(
            headers.get(USER_AGENT).unwrap(),
            HeaderValue::from_static(DEFAULT_USER_AGENT)
        );

        // Test 2: Should not override existing user-agent.
        let mut headers = HeaderMap::new();
        ...
        // Test 3: Apply custom request headers.
        ...
        // Test 4: Should not override existing custom headers.
        let mut headers = HeaderMap::new();
        headers.insert(
            "X-Custom-Header",
        ...
        // Test 5: Return error for invalid header name.
        let mut custom_headers = HashMap::new();
        custom_headers.insert("Invalid Header Name".to_string(), "value".to_string());
        let http = HTTP::new(HTTP_SCHEME, Some(custom_headers)).unwrap();
        let mut headers = HeaderMap::new();
        assert!(http.apply_request_headers(&mut headers).is_err());

        // Test 6: Return error for invalid header value.
        let mut custom_headers = HashMap::new();

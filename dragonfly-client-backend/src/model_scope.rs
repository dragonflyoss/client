...
mod tests {
    ...
    fn backend() -> ModelScope {
        ModelScope::new(Arc::new(Config::default())).unwrap()
    }

    fn options(server: &MockServer, token: Option<&str>) -> Option<ModelScopeOptions> {
        Some(ModelScopeOptions {
    ...
    fn stat_request(url: &str, model_scope: Option<ModelScopeOptions>) -> StatRequest {
        StatRequest {
    ...
    fn get_request(
        url: &str,
        range: Option<Range>,
        model_scope: Option<ModelScopeOptions>,
    ) -> GetRequest {
        GetRequest {
            task_id: "task".to_string(),
    ...
    fn exists_request(url: &str, model_scope: Option<ModelScopeOptions>) -> ExistsRequest {
        ExistsRequest {
            task_id: "task".to_string(),
            url: url.to_string(),
            http_header: None,
    ...
    async fn get_rejects_missing_options_or_file_path() {
        ...
        for (url, model_scope) in test_cases {
            let result = backend.get(get_request(url, None, model_scope)).await;
            assert!(matches!(result, Err(Error::InvalidParameter)));
        }
    }

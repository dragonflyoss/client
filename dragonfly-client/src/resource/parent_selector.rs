...
mod tests {
    ...
    async fn test_unregister_parents() {
        ...
        for (_, peers, setup_weights, should_succeed, should_remove_weights) in test_cases {
            let selector = ParentSelector::new(config.clone(), id_generator.clone()).unwrap();

            // Set up weights for this test case
            for (id, weight) in &setup_weights {
                selector.wetghts.insert(id.clone(), *weight);
            }

            let result = selector.unregister_parents(peers).await;

            if should_succeed {
                assert!(result.is_ok());

                // Check that weights were removed as expected
                for id in &should_remove_weights {
                    assert!(!selector.wetghts.contains_key(id));
                }
            } else {
                assert!(result.is_err());
            }

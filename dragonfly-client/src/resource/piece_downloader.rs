...
use dragonfly_client_storage::client::quic::QUICClient;
...
impl QUICDownloader {
    ...
    /// client returns the quic client by the address.
    ///
    /// Opterations:
    /// 1. If the client entry exists, it will return the client directly to reuse the client by
    ///    the address.
    /// 2. If the client entry does not exist, it will create a new client entry and insert it
    ///    into the clients map.
    async fn client(&self, addr: &str) -> Result<QUICClient> {
        let now = Instant::now();
        ...
        // clients for the same address. But it will reuse the same client by entry operation.
        debug!("creating client: {}", addr);
        let client = QUICClient::new(self.config.clone(), addr.to_string()).await?;
    ...
    /// get_client_entry returns the client entry by the address.
    async fn get_client_entry(&self, addr: &str) -> Option<QUICClientEntry> {
        let clients = self.clients.lock().await;
        clients.get(addr).cloned()
    }
...
impl Downloader for QUICDownloader {
    ...
    async fn download_piece(
        ...
        let quic_addr = if let Some((ip_str, port_str)) = addr.split_once(':') {

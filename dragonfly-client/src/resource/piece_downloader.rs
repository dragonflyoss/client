...
use dragonfly_client_util::pool::{Entry, Factory, Pool};
use std::io::Cursor;
use std::sync::Arc;
...
impl Factory<String, DfdaemonUploadClient> for DfdaemonUploadClientFactory {
    type Error = Error;
    async fn make_entry(&self, addr: &String) -> Result<DfdaemonUploadClient> {
...
impl GRPCDownloader {
    ...
    pub fn new(config: Arc<Config>, capacity: usize, idle_timeout: Duration) -> Self {
        ...
        Self {
            config,
            client_pool: Pool::new(factory, capacity, idle_timeout),
        }
    }
    ...
    async fn get_client_entry(&self, addr: &str) -> Result<Entry<DfdaemonUploadClient>> {
        self.client_pool.get_or_create(&addr.to_string()).await
    }

    /// remove_client_if_idle removes the client if it is idle.
    async fn remove_client_if_idle(&self, addr: &str) {
        self.client_pool.remove_if_idle(&addr.to_string()).await;
    }
}
    ...
    async fn download_piece(
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                drop(request_guard);
                self.remove_client_if_idle(&addr).await;
                return Err(err);
            }
        };
...
impl Factory<String, TCPClient> for TCPClientFactory {
    type Error = Error;
    async fn make_entry(&self, addr: &String) -> Result<TCPClient> {

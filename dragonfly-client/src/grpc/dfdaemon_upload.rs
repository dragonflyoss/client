...
use dragonfly_api::dfdaemon::v2::{
    ...
    StatCacheTaskRequest, StatCacheTaskRequest as DfdaemonStatCacheTaskRequest,
    StatPersistentCacheTaskRequest, StatTaskRequest, StatTaskRequest as DfdaemonStatTaskRequest,
    SyncCachePiecesRequest, SyncCachePiecesResponse, SyncHostRequest,
...
impl DfdaemonUpload for DfdaemonUploadServerHandler {
    ...
    async fn download_cache_task(
        ...
        let task_manager_clone = self.cache_task.clone();
        let task_clone = task.clone();
        let (out_stream_tx, out_stream_rx) = mpsc::channel(10 * 1024);
        ...
        if request.prefetch {
            ...
            match self
                .cache_task
                .prefetch_task_started(task_id.as_str())
                .await
            {
                Ok(_) => {
    ...
    async fn sync_cache_pieces(
        ...
        tokio::spawn(
            async move {
                let Ok(Some(_)) = task_manager.get(task_id.as_str()) else {
                    error!("get cache task {} not found", task_id);
                    out_stream_tx
                        .send_timeout(
                            Err(Status::internal(format!(
                                "cache task {} not found",
                                task_id
                            ))),
                            super::REQUEST_TIMEOUT,
                        )
                        .await
                        .unwrap_or_else(|err| {
                            error!("send cache task {} not found to stream: {}", task_id, err);
                        });

...
use dragonfly_api::common::v2::PersistentCacheTask;
...
pub enum PersistentCacheTaskSubCommand {
    ...
    #[command(
        ...
        author,
        version,
        about = "Remove a persistent cache task",
        long_about = "Remove a specific persistent cache task by ID from the local dfdaemon."
    )]
    Rm(RmCommand),
}
...
impl LsCommand {
    ...
    async fn run(
        ...
        let tasks = dfdaemon_download_client
...
/// RmCommand is the subcommand of persistent-cache-task rm.
#[derive(Debug, Clone, Parser)]
...
impl RmCommand {
    ...
    pub async fn execute(
        ...
        if let Err(err) = self.run(dfdaemon_download_client).await {
            match err {
                Error::TonicStatus(status) => {
                    println!(
                        "{}{}{}Removing Persistent Cache Task Failed!{}",
                        color::Fg(color::Red),
                        style::Italic,
                        style::Bold,
                        style::Reset,
    ...
    /// Runs the rm command to delete a persistent cache task by ID.
    async fn run(
        &self,
        dfdaemon_download_client: dragonfly_client::grpc::dfdaemon_download::DfdaemonDownloadClient,
    ) -> Result<()> {

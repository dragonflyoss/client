...
use dragonfly_api::common::v2::PersistentTask;
...
impl PersistentTaskCommand {
    pub async fn execute(self, endpoint: PathBuf, log_level: Level, console: bool) -> Result<()> {
        match self.subcommand {
            PersistentTaskSubCommand::Ls(cmd) => cmd.execute(endpoint, log_level, console).await,
            PersistentTaskSubCommand::Rm(cmd) => cmd.execute(endpoint, log_level, console).await,
        }
    }
}
...
impl RmCommand {
    ...
    async fn run(
        ...
        println!(
            "{}{}{}Persistent Task Removed Successfully!{}",

...
/// DEFAULT_SEED_PEERS_HEALTH_CHECK_INTERVAL is the default interval(5m) of health check for seed peers.
const DEFAULT_SEED_PEERS_HEALTH_CHECK_INTERVAL: Duration = Duration::from_secs(300);
...
impl SeedPeerSelector {
    ...
    pub async fn new(
        ...
        let health_check_interval =
            health_check_interval.unwrap_or(DEFAULT_SEED_PEERS_HEALTH_CHECK_INTERVAL);
    ...
    async fn refresh(&self) -> Result<()> {
        ...
        while let Some(result) = join_set.join_next().await {
            match result {
                Ok(Ok((id, peer))) => {
                    hosts.insert(id.clone(), peer);
                    hashring.add(id.clone());
                }
                Ok(Err(err)) => {
...
impl Selector for SeedPeerSelector {
    ...
    async fn select(&self, task_id: String, replicas: u32) -> Result<Vec<Host>> {
        ...
        let seed_peers = addrs
            .into_iter()
            .filter_map(|addr| seed_peers.hosts.get(&addr).cloned())
            .collect();
...
mod tests {
    ...
    async fn create_test_selector() -> SeedPeerSelector {
        ...
        SeedPeerSelector {
            health_check_interval: DEFAULT_SEED_PEERS_HEALTH_CHECK_INTERVAL,
    ...
    async fn add_test_host(selector: &SeedPeerSelector, addr: String, host: Host) {
        ...
        seed_peers.hosts.insert(addr.clone(), host);
        seed_peers.hashring.add(addr);
    }

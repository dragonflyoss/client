...
const DEFAULT_VNODES_PER_HOST: u32 = 3;
...
impl SeedPeerSelector {
    ...
    async fn refresh(&self) -> Result<()> {
        ...
        while let Some(result) = join_set.join_next().await {
            match result {
                Ok(Ok(peer)) => {
                    hashring.add(peer.ip.as_str(), peer.port as u16);
                    hosts.insert(format!("{}:{}", peer.ip, peer.port), peer);
                }
                Ok(Err(err)) => {
    ...
    async fn select(&self, task_id: String, replicas: u32) -> Result<Vec<Host>> {
        ...
        let seed_peers = vnodes
            ...
            .filter_map(|vnode| {
                seed_peers
                    .hosts
                    .get(vnode.get_addr().to_string().as_str())
                    .cloned()
            })
            .collect();
...
mod tests {
    ...
    async fn add_test_host(selector: &SeedPeerSelector, host: Host) {
        ...
        seed_peers.hashring.add(host.ip.as_str(), host.port as u16);
    }

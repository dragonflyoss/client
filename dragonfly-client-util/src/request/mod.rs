...
use crate::pool::{Entry, Factory, Pool};
...
impl Factory<String, Client> for HTTPClientFactory {
    type Error = Error;
    async fn make_entry(&self, proxy_addr: &String) -> Result<Client> {
...
impl Proxy {
    pub fn builder(scheduler_endpoint: impl Into<String>) -> ProxyBuilder {
        ProxyBuilder {
...
impl ProxyBuilder {
    /// with_health_check_interval sets the health check interval.
    pub fn with_health_check_interval(mut self, interval: Duration) -> Self {
        self.health_check_interval = interval;
        self
    }

    /// with_max_retries sets the maximum number of retries.
...
impl Request for Proxy {
    ...
    async fn get_into(&self, request: GetRequest, buf: &mut BytesMut) -> Result<GetResponse> {
        ...
        // Check if timeout is zero, if so, don't apply timeout (unlimited wait).
...
impl Proxy {
    ...
    async fn client_entries(&self, request: &GetRequest) -> Result<Vec<Entry<Client>>> {
        ...
        for peer in seed_peers.iter() {
            ...
            let client_entry = self.client_pool.get_or_create(&proxy_addr).await?;
    ...
    /// Private helper to process requests and handle response headers.
    #[instrument(skip_all)]
    async fn process(&self, request: &GetRequest) -> Result<reqwest::Response> {
        ...
        for entry in entries.iter() {
            match self.send(entry, request, &headers).await {
        ...
        Err(last_error.unwrap_or_else(|| {
            DFError::Unknown("all client entries failed but no error recorded".to_string()).into()
        }))
    }
...
mod tests {
    ...
    async fn test_proxy_new_success() -> Result<()> {
        ...
        let result = Proxy::builder(&scheduler_endpoint).build().await;
    ...
    async fn test_proxy_new_empty_endpoint() {
        let result = Proxy::builder("".to_string()).build().await;
    ...
    async fn test_proxy_new_invalid_retry_times() -> Result<()> {
        ...
        let scheduler_endpoint = format!("http://0.0.0.0:{}", mock_server.port().unwrap());
        let result = Proxy::builder(&scheduler_endpoint)
            .with_max_retries(11)
            .build()
            .await;
    ...
    async fn test_proxy_new_invalid_health_check_interval() -> Result<()> {
        ...
        let scheduler_endpoint = format!("http://0.0.0.0:{}", mock_server.port().unwrap());
        let result = Proxy::builder(&scheduler_endpoint)
            .with_health_check_interval(Duration::from_secs(1000))
            .build()
            .await;
    ...
    async fn test_client_pool_get_or_create() {
        let pool = Pool::new(HTTPClientFactory {}, 10, Duration::from_secs(600));
        ...
        let _ = pool
            .get_or_create(&"http://proxy1.com".to_string())
            .await
            .unwrap();
        ...
        let _ = pool
            .get_or_create(&"http://proxy2.com".to_string())
            .await
            .unwrap();
    ...
    async fn test_client_pool_cleanup() {
        let pool = Pool::new(HTTPClientFactory {}, 10, Duration::from_millis(10));

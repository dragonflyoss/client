    ...
    async fn test_proxy_new_success() {
        ...
        match mock_server {
            Ok(mock_server) => {
                ...
                let result =
                    Proxy::new(scheduler_endpoint, Some(Duration::from_secs(60)), Some(2)).await;

                assert!(result.is_ok());
                let proxy = result.unwrap();
                assert_eq!(proxy.retry_times, 2);
            }
            Err(err) => panic!("Failed to start mock server: {}", err),
        };
    }
    ...
    async fn test_proxy_new_invalid_retry_times() {
        ...
        match mock_server {
            Ok(mock_server) => {
                let scheduler_endpoint = format!("http://0.0.0.0:{}", mock_server.port().unwrap());
                let result =
                    Proxy::new(scheduler_endpoint, Some(Duration::from_secs(60)), Some(5)).await;

...
impl VNodeHashRing {
    ...
    pub fn add(&mut self, ip: &str, port: u16) {
        for id in 0..self.virtual_nodes_count {
            let vnode = VNode::new(ip, port, id as usize);
            self.ring.add(vnode);
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_vnode_new() {
        let vnode = VNode::new("127.0.0.1", 8080, 1);
    ...
    fn test_hashring_new() {
        ...
        assert_eq!(ring.virtual_nodes_count, 3);
    ...
    fn test_add_and_len() {
        ...
        ring.add("192.168.1.1", 8080);
        assert_eq!(ring.len(), 2); // 1 node * 2 virtual nodes
        ring.add("192.168.1.2", 8081);
